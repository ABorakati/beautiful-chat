...
const hubMessage = {
 ...
 data: {
  ...
  from: "Main",
  to: "ShotBuilder",
  message: "Capture the dark theme first. Light theme after.",
  delivered: true,
  awaitReply: false,
 },
};
...
const hubProcess = {
 ...
 data: {
  ...
  args: ["run", "serve"],
  port: 4173,
  readyLogPattern: "Local:.*http",
  status: "ready" as const,
  recentLogs: ["bun serve", "Local: http://127.0.0.1:4173"],
 },
};
...
const tasks = {
 ...
 tasks: [
  ...
  { id: "t3", title: "Capture every component", phase: "Screenshots", status: "pending" as const },
  {
   id: "t4",
...
function Shot({ id, width = 900, children }: { id: string; width?: number; children: React.ReactNode }) {
 return (
 ...
 return (
  <div style={{ display: "flex", flexDirection: "column", gap: 24, backgroundColor: "#000" }}>
   ...
   <Shot id="shot-hub">
    ...
    <HubCallout data={hubProcess} tokens={tokens} />
   </Shot>
   <Shot id="shot-paseo">
...
if (mount) { try { createRoot(mount).render(<Showcase />); console.log("showcase: render called"); } catch (error) { console.error("showcase: render threw", error); } }
