...
for (const [name, uri] of Object.entries(DEVICON_URIS)) {
  marks.push({ id: `devicon:${name}`, svg: decodeURIComponent(uri.replace(/^data:image\/svg\+xml,/, "")), mono: false });
}
...
for (const [name, d] of [["file", GENERIC_FILE_PATH], ["folder", FOLDER_PATH]] as const) {
  marks.push({
    id: `mark:${name}`,
