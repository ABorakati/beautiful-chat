...
/**
 * Brand marks the plugin draws itself. Each carries its own viewBox, because
 * the Paseo mark is authored on a 700 unit grid while the others use 24.
 */
const BRAND_PATHS = {
  Git: {
    viewBox: "0 0 24 24",
...
function brandUri(name: keyof typeof BRAND_PATHS, color: string): string {
  ...
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="${brand.viewBox}" ` +
    `fill="${color}"><path d="${brand.d}"/></svg>`;
