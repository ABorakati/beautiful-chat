...
import { LOBE_MARKS } from "./lobe-marks";
 ...
 */
const LOCAL_MARKS: Partial<Record<KnownProvider, string>> = {
  paseo:
...
function markUri(provider: KnownProvider, color: string): string | undefined {
  const local = LOCAL_MARKS[provider];
  const vendor = LOBE_MARKS[provider];
  const svg = local ?? vendor?.svg;
  if (!svg) return undefined;

  const key = `${provider}|${color}`;
  const cached = uriCache.get(key);
  if (cached) return cached;

  const mono = local ? true : (vendor?.mono ?? false);
  const painted = mono ? svg.replaceAll("currentColor", color) : svg;
  const uri = `data:image/svg+xml,${encodeURIComponent(painted)}`;
...
export function ProviderLogo({ provider, size = 14, showLabel = false }: ProviderLogoProps) {
  ...
  const uri = useMemo(
    () => markUri(normalized, config.brandColor),
    [normalized, config.brandColor],
  );
